...
def sum_coeff(dat, n, M, bs, Pattern, dev):
    ...
    for inputs, labels in dat:
        ...
        if j < N:
            S[j * bs: (j + 1) * bs] = torch.sum(x,2)

        else:
            S[j * bs:] = torch.sum(x, 2)
...
fig, ax = plt.subplots()
X_plot = np.linspace(np.min(cumsum_coef), np.max(cumsum_coef), 1000)[:, np.newaxis]
bins = np.linspace(np.min(cumsum_coef), np.max(cumsum_coef), 20)
log_dens = kde.score_samples(X_plot)
ax.fill(X_plot[:, 0], np.exp(log_dens), fc='#AAAAFF')
ax.text(-3.5, 0.31, "Gaussian Kernel Density")

# np.save(precompute_root / 'NVMS_{}_CR_{}_batch_size_{}.npy'.format(N0_NVMS, CR, batch_size), Noise_Variance)
# print('Saved : NVMS_{}_CR_{}_batch_size_{}.npy'.format(N0_NVMS, CR, batch_size))