...
if __name__ == "__main__":
    ...
    if opt.NVMS:
        NVMS_root = Path('/home/amador/Documents/Stage/Codes/Semaine15/NVMS/')

        My_NVMS_file = Path(NVMS_root) / (
            'NVMS_N0_{}_N_{}_M_{}.npy'.format(np.int(opt.intensity_max), opt.img_size, opt.CR))
        NVMS = np.load(My_NVMS_file) / (opt.intensity_max * (1 - opt.intensity_sig)) ** 2
        print('loaded :NVMS_N0_{}_N_{}_M_{}.npy'.format(np.int(opt.intensity_max), opt.img_size, opt.CR))
