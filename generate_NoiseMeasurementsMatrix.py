...
precompute_root = Path('/home/amador/Documents/Stage/Codes/Semaine15/NVMS/')
...
N0 = 100  # Photon level measurement
...
def forward_acquisition(x, b, c, h, w, m, alpha, alpha_var, Pattern):
    ##############################################################
    # --Scale input image to [0,1] and adjust it by de noise level
    ##############################################################
    a = alpha * (1 + alpha_var * (torch.rand(x.shape[0]) - 0.5)).to(x.device)
...
def NVMS(m, alpha, alpha_var, Pattern):
    ...
    for inputs, labels in dataloader:
        ...
        x = forward_acquisition(inputs, b, c, h, w, m, alpha, alpha_var, Pattern)
...
Noise_Variance = NVMS(m=CR, alpha=N0, alpha_var=sig, Pattern=P).cpu().detach().numpy()
Noise_Variance = np.reshape(Noise_Variance, (CR,))
Noise_Variance = np.diag(Noise_Variance)

np.save(precompute_root / 'NVMS_N0_{}_N_{}_M_{}.npy'.format(N0, img_size, CR), Noise_Variance)
print('Saved : NVMS_N0_{}_N_{}_M_{}.npy'.format(N0, img_size, CR))
